...
MixtrackPlatinumFX.init = function(id, debug) {
    ...
    // exit demo mode
    var byteArray = [0xf0, 0x00, 0x20, 0x7f, 0x04, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7];
	var byteArray = [0xf0, 0x7e, 0x00, 0x06, 0x01, 0xf7];
...
MixtrackPlatinumFX.Sampler.prototype = new components.ComponentContainer();

MixtrackPlatinumFX.HeadGain = function(sampler) {
    components.Pot.call(this);
...
MixtrackPlatinumFX.HeadGain.prototype = new components.Pot({
    group: '[Master]',
    inKey: 'headGain',
    input: function (channel, control, value, status, group) {
...
MixtrackPlatinumFX.BrowseKnob = function() {
...
MixtrackPlatinumFX.BrowseKnob.prototype = new components.ComponentContainer();
...
MixtrackPlatinumFX.elapsedToggle = function (channel, control, value, status, group) {
    if (value != 0x7F) return;
...
// still spinning
MixtrackPlatinumFX.scratch_timer = []; // initialized before use (null is an acceptable value)
MixtrackPlatinumFX.scratch_tick = [];  // initialized before use
MixtrackPlatinumFX.resetScratchTimer = function (deck, tick) {
    if (!MixtrackPlatinumFX.scratch_timer[deck]) return;
    MixtrackPlatinumFX.scratch_tick[deck] = tick;
};

MixtrackPlatinumFX.startScratchTimer = function (deck) {
    if (MixtrackPlatinumFX.scratch_timer[deck]) return;

    MixtrackPlatinumFX.scratch_tick[deck] = 0;
    MixtrackPlatinumFX.scratch_timer[deck] = engine.beginTimer(20, function() {
        MixtrackPlatinumFX.scratchTimerCallback(deck);
    });
};

MixtrackPlatinumFX.stopScratchTimer = function (deck) {
    if (MixtrackPlatinumFX.scratch_timer[deck]) {
        engine.stopTimer(MixtrackPlatinumFX.scratch_timer[deck]);
    }
    MixtrackPlatinumFX.scratch_timer[deck] = null;
};

MixtrackPlatinumFX.scratchTimerCallback = function (deck) {
    ...
    if ((MixtrackPlatinumFX.scratch_direction[deck] // forward
            && Math.abs(MixtrackPlatinumFX.scratch_tick[deck]) > 2)
        || (!MixtrackPlatinumFX.scratch_direction[deck] // backward
            && Math.abs(MixtrackPlatinumFX.scratch_tick[deck]) > 1))
    {
    ...
    MixtrackPlatinumFX.scratchDisable(deck);
};

MixtrackPlatinumFX.scratchDisable = function (deck) {
    MixtrackPlatinumFX.searching[deck] = false;
    MixtrackPlatinumFX.stopScratchTimer(deck);
    engine.scratchDisable(deck, false);
};

MixtrackPlatinumFX.scratchEnable = function (deck) {
    var alpha = 1.0/8;
    var beta = alpha/32;
...
MixtrackPlatinumFX.touching = [false, false, false, false, false];
MixtrackPlatinumFX.searching = [false, false, false, false, false];
MixtrackPlatinumFX.wheelTouch = function (channel, control, value, status, group) {
    var deck = channel + 1;

    // ignore touch events if not in vinyl mode
    if (!MixtrackPlatinumFX.shift
        && !MixtrackPlatinumFX.searching[deck]
        && !MixtrackPlatinumFX.wheel[channel]
        && value != 0)
    {
        return;
    }

    MixtrackPlatinumFX.touching[deck] = 0x7F == value;
    ...
    if (value === 0x7F
        ...
        && !MixtrackPlatinumFX.searching[deck])
    {
        MixtrackPlatinumFX.scratchEnable(deck);
    }
    else if (value === 0x7F
             && (MixtrackPlatinumFX.shift
                || MixtrackPlatinumFX.searching[deck]))
    {
        MixtrackPlatinumFX.scratchDisable(deck);
        MixtrackPlatinumFX.searching[deck] = true;
        MixtrackPlatinumFX.stopScratchTimer(deck);
    }
    else {    // If button up
        MixtrackPlatinumFX.startScratchTimer(deck);
    }
};

// The wheel that actually controls the scratching
// indexed by deck numbers starting at 1, so include an extra element
MixtrackPlatinumFX.scratch_direction = [null, null, null, null, null]; // true == forward
MixtrackPlatinumFX.scratch_accumulator = [0, 0, 0, 0, 0];
MixtrackPlatinumFX.last_scratch_tick = [0, 0, 0, 0, 0];
MixtrackPlatinumFX.wheelTurn = function (channel, control, value, status, group) {
    ...
    // to be
    var delta = Math.abs(MixtrackPlatinumFX.last_scratch_tick[deck] - value);
    if (MixtrackPlatinumFX.scratch_direction[deck] !== null && MixtrackPlatinumFX.scratch_direction[deck] != direction && delta < 64) {
        direction = !direction;
    }
    ...
    if (MixtrackPlatinumFX.searching[deck]) {
        var position = engine.getValue(group, 'playposition');
        if (position <= 0) position = 0;
        if (position >= 1) position = 1;
        engine.setValue(group, 'playposition', position + newValue * 0.0001);
        MixtrackPlatinumFX.resetScratchTimer(deck, newValue);
        return;
    }

    // stop scratching if the wheel direction changes and the platter is not
    // being touched
    if (MixtrackPlatinumFX.scratch_direction[deck] === null) {
        MixtrackPlatinumFX.scratch_direction[deck] = direction;
    }
    else if (MixtrackPlatinumFX.scratch_direction[deck] != direction) {
        if (!MixtrackPlatinumFX.touching[deck]) {
            MixtrackPlatinumFX.scratchDisable(deck);
        }
        MixtrackPlatinumFX.scratch_accumulator[deck] = 0;
    }

    MixtrackPlatinumFX.last_scratch_tick[deck] = value;
    MixtrackPlatinumFX.scratch_direction[deck] = direction;
    MixtrackPlatinumFX.scratch_accumulator[deck] += Math.abs(newValue);
    ...
    // handle beat jumping
    else if (MixtrackPlatinumFX.shift) {
        if (MixtrackPlatinumFX.scratch_accumulator[deck] > 61) {
            MixtrackPlatinumFX.scratch_accumulator[deck] -= 61;
...
MixtrackPlatinumFX.wheel = []; // initialzed in the MixtrackPlatinumFX.init() function
MixtrackPlatinumFX.wheelToggle = function (channel, control, value, status, group) {
    if (value != 0x7F) return;
    MixtrackPlatinumFX.wheel[channel] = !MixtrackPlatinumFX.wheel[channel];
    var on_off = 0x01;
    if (MixtrackPlatinumFX.wheel[channel]) on_off = 0x7F;
    midi.sendShortMsg(0x90 | channel, 0x07, on_off);
};

MixtrackPlatinumFX.deckSwitch = function (channel, control, value, status, group) {
    var deck = channel + 1;
    MixtrackPlatinumFX.decks[deck].setActive(value == 0x7F);

    // change effects racks
    if (MixtrackPlatinumFX.decks[deck].active && (channel == 0x00 || channel == 0x02)) {
        MixtrackPlatinumFX.effects[1].setCurrentUnit(deck);
    }
    else if (MixtrackPlatinumFX.decks[deck].active && (channel == 0x01 || channel == 0x03)) {
        MixtrackPlatinumFX.effects[2].setCurrentUnit(deck);
    }
...
MixtrackPlatinumFX.pflToggle = function(value, group, control) {
    midi.sendShortMsg(0xBF, 0x44, 0);
    midi.sendShortMsg(0xBF, 0x45, 0);
};
...
MixtrackPlatinumFX.vuCallback = function(value, group, control) {
        ...
        || engine.getValue('[Channel4]', 'pfl'))
    {
        ...
        if (group == '[Channel1]' && MixtrackPlatinumFX.decks[1].active) {
            midi.sendShortMsg(0xBF, 0x44, level);
        }
        else if (group == '[Channel3]' && MixtrackPlatinumFX.decks[3].active) {
            midi.sendShortMsg(0xBF, 0x44, level);
        }
        else if (group == '[Channel2]' && MixtrackPlatinumFX.decks[2].active) {
            midi.sendShortMsg(0xBF, 0x45, level);
        }
        else if (group == '[Channel4]' && MixtrackPlatinumFX.decks[4].active) {
            midi.sendShortMsg(0xBF, 0x45, level);
        }
    }
...
MixtrackPlatinumFX.shift = false;
MixtrackPlatinumFX.shiftToggle = function (channel, control, value, status, group) {
    MixtrackPlatinumFX.shift = value == 0x7F;

    if (MixtrackPlatinumFX.shift) {
        MixtrackPlatinumFX.decks.shift();
        MixtrackPlatinumFX.sampler_all.shift();
        MixtrackPlatinumFX.effects.shift();
        MixtrackPlatinumFX.browse.shift();
        MixtrackPlatinumFX.head_gain.shift();

        // reset the beat jump scratch accumulators
        MixtrackPlatinumFX.scratch_accumulator[1] = 0;
        MixtrackPlatinumFX.scratch_accumulator[2] = 0;
        MixtrackPlatinumFX.scratch_accumulator[3] = 0;
        MixtrackPlatinumFX.scratch_accumulator[4] = 0;
    }
    else {
        MixtrackPlatinumFX.decks.unshift();
        MixtrackPlatinumFX.sampler_all.unshift();
        MixtrackPlatinumFX.effects.unshift();
        MixtrackPlatinumFX.browse.unshift();
        MixtrackPlatinumFX.head_gain.unshift();
    }
};
